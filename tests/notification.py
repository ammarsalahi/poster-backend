...
@pytest.mark.asyncio
async def test_list_notifies(test_notifications,get_Header,get_Header_admin,override_get_db):
    transport = ASGITransport(app=app)
    async with AsyncClient(transport=transport,base_url=BASE_URL) as client:
        response = await client.get("/notifies/?limit=10&offset=0",headers=get_Header_admin)
        print(f"details:  {response.text}")
        assert response.status_code ==200
        
        #without query paramerters
        # wo_response = await client.get("/notifies/",headers=get_Header_admin)

        # assert wo_response.status_code ==200

        # #without auth 
        # auth_response = await client.get("/notifies/")
...
# @pytest.mark.asyncio
# async def test_detail_comments(test_notifications,get_Header_admin,override_get_db):
#     transport = ASGITransport(app=app)
#     async with AsyncClient(transport=transport,base_url=BASE_URL) as client:
#         id=test_notifications[0].id
        
#         response = await client.get(f"/notifies/{id}",headers=get_Header_admin)
#         print(f"d:   {response.text}")
#         assert response.status_code ==200

#         #invalid
#         invalid_response = await client.get("/notifies/mio",headers=get_Header_admin)
#         print(invalid_response.text)
#         assert invalid_response.status_code ==422


#         auth_response = await client.get(f"/notifies/{id}")
#         assert auth_response.status_code ==401        
