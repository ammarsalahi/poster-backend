...
async def create_media(session:sessionDep,currentUser:userDep,media_data:MediaAdd):
    if currentUser:
        return await MediaCrud(session).add(media_data)


@routers.patch("/{id}",response_model=MediaResponse)
async def update_media(session:sessionDep,currentUser:userDep,media_id:UUID,media_data:MediaEdit):
